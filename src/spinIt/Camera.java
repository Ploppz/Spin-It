...
/**
 * This version of camera will approach a target size of the object it follows.
 */
...
// FINDING THE BOUNDING RECTANGLE {
		...
		for (GameObject toFollow : followList) {
			if (toFollow.position.x < bound.x) {
				bound.x = (int) (toFollow.position.x - toFollow.size.x/2);
			}
			if (toFollow.position.y < bound.y) {
				bound.y = (int) (toFollow.position.y - toFollow.size.y/2);
			}
		...
		for (GameObject toFollow : followList) {
			int relativeX = (int) ((toFollow.position.x + toFollow.size.x/2) - bound.x); // relative to bound's x
			int relativeY = (int) ((toFollow.position.y + toFollow.size.y/2) - bound.y); // ...
...
//	}
		...
		if (followList.size() > 0) {
			targetZoom = (200 / followList.get(0).size.x);
